...
class line_follow():
    ...
    def __init__(self):
        ...
        self.last_encoder_for_stop_  = 0
        self.last_encoder_for_stop_1 = 0
        #self.lane_to_turn = 1
...
#        server_sub = rospy.Subscriber('linedetectionctrl', Int32, self.server_callback)
        ...
        self.line_pub = rospy.Publisher('line_enable',Int32,queue_size = 100)
                    ...
                    if self.stop_flag == 1:
                        if self.take_pallet == 0:
                            if self.has_sub_line == "yes":
                                self.take_pallet = 1
                                print("encoder_1111111",self.last_encoder_for_stop)
                            ...
                            if self.cross_detect == 1 and self.lane_count == self.lane_to_turn :#uncomment here when done
                                ...
                                if self.balance_flag == 1:
                                    if self.turn_flag == 0:
                                        if self.pos_left == 1:
                                            ...
                                            if pos == 8 and self.count_magss < 8 and self.line_flag == 0:
                                                ...
                                                if self.count_magss > 9 and self.loss_line_temp_3 == 0  :
                                                    self.lane_count += 1
                                    ...
                                    #print("self.lane_count = ",self.lane_count)
                                    if self.count_magss > 9 and self.loss_line_temp_3 == 0  :
                                        self.lane_count += 1
                                        self.loss_line_temp_3 = 1
                                    if self.loss_line_temp_3 == 1:
                                        if self.count_magss > 9:
                                            ...
                                            self.loss_line_temp_3 = 0    
#                            if self.cross_detect == 1 and self.lane_count == self.lane_to_turn:# and self.pos_stop == 1 :#uncomment here when done
#                                #self.vel_pub.publish(0)
#                                #self.ste_pub.publish(5200)
#                                self.stop_encoder = -(self.t_enc)
...
#                                        self.loss_line_temp_3 = 0                                     
                            ...
                            if self.count_2 > 20:
                                ...
                                self.encoder_for_stop = (-((self.last_encoder_for_stop_1) + (self.t_enc))) + self.encoder_for_stop_ + 100
                                print("encoder_stop_22222=",self.encoder_for_stop)
                                self.vel_pub.publish(0)
                                self.ste_pub.publish(self.home_value)
                            ...
                            if self.cross_detect == 1 and self.lane_count_ == self.lane_to_turn_ :#uncomment here when done
                                ...
                                if self.balance_flag == 1:
                                    ...
                                    self.angle_controll(-1150)  
                                    ##print("self.lane_count = ",self.lane_count)
                                    if self.count_magss > 9 and self.loss_line_temp_4 == 0  :
                                        self.lane_count_ += 1
                                        self.loss_line_temp_4 = 1
                                    if self.loss_line_temp_4 == 1:
                                        if self.count_magss > 9:
                                            pass
                                        else:
                                            self.loss_line_temp_4 = 0     
...
#                                        self.loss_line_temp_4 = 0  
                            ...
                            if self.count_2 > 30:
                                ...
                                self.encoder_for_stop = (-((self.last_encoder_for_stop_1) + (self.t_enc))) + self.encoder_for_stop_ + 100
                                #print("encoder_222222222",self.last_encoder_for_stop)
                                print("encoder_stop_66666=",self.encoder_for_stop)
                                self.vel_pub.publish(0)
                                self.ste_pub.publish(self.home_value)
                            ...
                            if self.has_sub_line == "yes":
                                ...
                                if self.pallet == 0:
                                    ...
                                    if self.lift_val == 1:
                                        self.lift_pub.publish("lift_stop")
                                        self.take_pallet = 16
                                        self.last_encoder_for_stop_ = -(self.t_enc)
...
#                                self.angle_controll_front(1250)
                            ...
                            if self.cross_front_detect and self.lane_count_out == self.lane_to_turn_ :
                                ...
                                self.angle_controll_front(1250)
                                if self.count_front_magss > 9 and self.loss_line_temp_4 == 0  :
                                    self.lane_count_out += 1
                                    self.loss_line_temp_4 = 1
                                if self.loss_line_temp_4 == 1:
                                    if self.count_magss > 9:
                                        pass
                            ...
                            print("encoder_stop_16161616=",self.encoder_for_stop)
                            if ((self.last_encoder_for_stop_) + (self.t_enc)) >= self.encoder_for_stop:
                                self.vel_pub.publish(0)
                                self.ste_pub.publish(self.home_value)
                                self.take_pallet = 17
                            else:
                            ...
                            if self.dir_out == 3:
                                ...
                                self.flag_2 = 1
                                ##print("encoder_here",((self.last_encoder_1) + (self.t_enc)))
                                if ((self.last_encoder_1) + (self.t_enc)) > 2000:
                                    self.vel_pub.publish(0)
                                    self.ste_pub.publish(self.home_value)
                                    self.take_pallet = 19
                                else:
                        ...
                        if self.cross_detect == 1 and self.lane_count == self.lane_to_turn_ and self.flag_3 == 0:#uncomment here when done
                            self.flag_3 = 1
                        elif self.flag_3 == 1:
                            self.now_encoder = -(self.t_enc)
                            if self.has_sub_line == "yes":
                                ...
                                self.flag = 1
                                self.flag_3 = 0
                                self.encoder_for_stop_ = -((self.last_encoder_for_stop) + (self.t_enc))
                                self.last_encoder_for_stop_1 = -(self.t_enc)
                                #print("encoder_222222222",self.last_encoder_for_stop)
                                #self.stop_flag = 1
                            else:
                                self.take_pallet = 6
