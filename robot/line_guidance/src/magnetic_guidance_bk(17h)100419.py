...
#        self.logger.setLevel(logging.INFO)
        ...
        manual = rospy.Subscriber('/manual',Int32 , self.manual_callback)
...
#        self.reset_run_pub = rospy.Publisher('reset_run_button',Int32,queue_size = 10)
    ...
    def server_charger_callback(self,msg):  
        data = msg.data
        print("server_charger_callback",data)
        if data == 1206:
            self.PID_enable = 6   
        elif data == 1207:
            self.PID_enable = 7                  
...
#        self.t_enc = msg.x
    ...
    def pallet_pos_callback(self,msg):
        print("pallet_pos_callback",msg.data)
        data = msg.data
        if data == 1205:
            self.pos_stop = 1
        else:
    ...
    def loss_line(self):
        ...
        if self.loss_line_temp == 0:
            ...
            if self.has_sub_line == "no" and self.line_ord == 1:
                ...
                if ((self.last_encoder) + (self.t_enc)) < int(-2300*self.encoder_var) :
                    for i in range(30):
                ...
                if self.count_magss > 3 :
                    self.loss_line_temp = 3
                elif ((self.last_encoder) + (self.t_enc)) < int(-2300*self.encoder_var) :
                    for i in range(30):
                        self.vel_pub.publish(0)
                        self.ste_pub.publish(self.home_value)
                    self.PID_enable = 2
...
#            self.loss_line_temp = 3
            ...
            if self.dir_main == 1:
                if self.count_2 == 0:
                    ...
                    if ((self.last_encoder_3) + (self.t_enc)) < int(-215*self.encoder_var):
                        for i in range(5):
                            self.vel_pub.publish(0)
                            self.ste_pub.publish(self.home_value)
                        for i in range(20):
                ...
                if self.count_2 == 0:
                    ...
                    if ((self.last_encoder_3) + (self.t_enc)) < int(-50*self.encoder_var):
                        self.vel_pub.publish(0)
                        self.ste_pub.publish(self.home_value)
                        self.count_2 = 2
                        self.last_encoder_3 = -(self.t_enc)
...
#                    #print("(self.last_encoder_3) + (self.t_enc)",(self.last_encoder_3) + (self.t_enc))
                    if ((self.last_encoder_3) + (self.t_enc)) > int(15*self.encoder_var):
                        for i in range(20):
                            self.vel_pub.publish(0)
                            self.ste_pub.publish(self.home_value)
                        for i in range(20):
            ...
            if self.dir_main == 1:
                if ((self.last_encoder_1) + (self.t_enc)) > int(1100*self.encoder_var):
    ...
    def loss_line_charger(self):
        ...
        if self.loss_line_temp == 0:
            ...
            if ((self.last_encoder_1) + (self.t_enc)) < int(-800*self.encoder_var):
                self.loss_line_temp = 5
                self.vel_pub.publish(-1300)
                self.ste_pub.publish(2000)
            else:
                        ...
                        if self.take_pallet == 0:
                                    ...
                                    if self.turn_flag == 0:
                                        if self.pos_left == 1:
                                            if self.bay_count == (self.bay - 1):
                                                if pos == 8 and self.count_magss < 8 and self.line_flag == 0 and self.cross_detect == 0 :
                                                    if self.bay_count == (self.bay - 1):
                                                        self.temp_1 = 0
                                                        self.balance_flag = 0
                                                        self.turn_flag = 0
                                        ...
                                        if self.temp_1 == 1 :
                                            if ((self.now_encoder) + (self.t_enc)) <= int((self.time + 15)*self.encoder_var) :
                                                self.temp_1 = 0
                                                self.balance_flag = 0
                                                self.turn_flag = 0
                                                self.loss_line_temp_3 = 0
                            ...
                            if self.dir_sub == 1:
                                if self.count_2 == 0:
                                    ...
                                    self.count_3 += 1
                                    if ((self.last_encoder_3) + (self.t_enc)) < int(-215*self.encoder_var) or self.count_3 >=42:
                            ...
                            if self.count_6 == 0:
                                ...
                                if ((self.temp_enc) + (self.t_enc)) < int(-700*self.encoder_var):
                                    self.take_pallet = 5
                                    self.count_6 = 2
                                    self.count_9 = 0
                                    self.flag_2 = 0
...
#                                    self.ste_pub.publish(self.home_value)
#                                    self.take_pallet = 11
                                if ((self.last_encoder_2) + (self.t_enc)) >= int(1200*self.encoder_var) :
                                    self.vel_pub.publish(0)
                                    self.ste_pub.publish(self.home_value)
                                    self.take_pallet = 12
                                else:
...
#                                    self.ste_pub.publish(self.home_value)
#                                    self.take_pallet = 11
                                if ((self.last_encoder_2) + (self.t_enc)) >= int(1400*self.encoder_var) :
                                    self.vel_pub.publish(0)
                                    self.ste_pub.publish(self.home_value)
                                    self.take_pallet = 12
                                else:
                            ...
                            self.ste_pub.publish(2000)
                            self.take_pallet = 13
                            self.last_encoder_90_d = -(self.t_enc)
...
#                            print("((self.last_encoder_4) + (self.t_enc))",((self.last_encoder_4) + (self.t_enc)))
                            if self.dir_sub == 2:
                                if ((self.last_encoder_4) + (self.t_enc)) > int(1000*self.encoder_var) :# orself.mag_left_value == 1 :
                                    self.vel_pub.publish(1000)
                                    self.ste_pub.publish(2000)
                                    self.take_pallet = 15
                                    self.last_encoder_4 = 0
                                ...
                                if ((self.last_encoder_4) + (self.t_enc)) < int(-1000*self.encoder_var): # or self.mag_right_value == 1 
                                    self.vel_pub.publish(-1000)
                                    self.ste_pub.publish(2000)
                                    self.take_pallet = 15
                                    self.last_encoder_4 = 0
                            ...
                            if self.dir_sub == 2:
                                #self.last_encoder_5 += 1
                                print("((self.last_encoder_90_d) + (self.t_enc))",((self.last_encoder_90_d) + (self.t_enc)))
                                if ((self.last_encoder_90_d) + (self.t_enc)) >= (self.enc_90 - 100) and ((self.last_encoder_90_d) + (self.t_enc)) <= (self.enc_90 + 200):# or self.last_encoder_5 > 200 :
                                    print("self.position_front(self.mag_ss_front)",self.position_front(self.mag_ss_front))
                                    print("self.mag_ss_front",self.mag_ss_front)
                                    if (self.position_front(self.mag_ss_front) >= 2 and self.position_front(self.mag_ss_front) <= 15 and self.no_line_flag_front == 0) or (self.cross_front_detect == 1) :
                                        self.vel_pub.publish(0)
                                        self.ste_pub.publish(self.home_value)
                                        self.front_flag = 0
                                        self.front_flag_ = 0
                                ...
                                print("self.position_front(self.mag_ss_front)",self.position_front(self.mag_ss_front))
                                print("self.mag_ss_front",self.mag_ss_front)
                                if self.position_front(self.mag_ss_front) >= 6 and self.position_front(self.mag_ss_front) <= 11  and self.no_line_flag_front == 0 :
                                    #print("ok here",self.last_encoder_5)
                                    self.vel_pub.publish(0)
                                    self.ste_pub.publish(self.home_value)
                                    self.take_pallet = 16
                                else:
                            ...
                            if self.dir_out == 3:
                                ...
                                if ((self.last_encoder_1) + (self.t_enc)) < int(-1700*self.encoder_var):
                                    self.vel_pub.publish(0)
                                    self.ste_pub.publish(self.home_value)
                                    self.take_pallet = 22
                                else:
                        ...
                        if ((self.now_encoder) + (self.t_enc)) <= int(self.time*self.encoder_var) :
                            self.stop_flag = 1
                            self.temp_1 = 0
                            self.loss_line_flag_1 = 0
...
#                        self.take_pallet = 1
                    ...
                    self.flag_2 = 1
                    if ((self.now_encoder) + (self.t_enc)) > int(500*self.encoder_var):
                        for i in range(30):
                            self.vel_pub.publish(0)
                            self.ste_pub.publish(self.home_value)
                        self.take_pallet = 3
...
#            self.reset_run_pub.publish(2000)
            ...
            if ((self.last_encoder_1) + (self.t_enc)) > int(270*self.encoder_var):
                self.temp_1 = 2
                self.PID_enable = 2
                self.charger_flag = 1
...
#                self.ste_pub.publish(2000) 
            ...
            if ((self.last_encoder_1) + (self.t_enc)) < int(-3000*self.encoder_var):
                self.temp_1 = 3
                self.vel_pub.publish(0)
                self.ste_pub.publish(self.home_value)
            else:
            ...
            if ((self.last_encoder_1) + (self.t_enc)) > int(1000*self.encoder_var):
                for i in range(30):
                    self.vel_pub.publish(0)
                    self.ste_pub.publish(self.home_value)
                self.temp_1 = 6
