...
#                                        self.turn_flag = 0
                            ...
                            if self.dir_sub == 1:
                                if self.count_2 == 0:
                                    ...
                                    self.count_3 += 1
                                    if ((self.last_encoder_3) + (self.t_enc)) < int(-197*self.encoder_var) or self.count_3 >=39:
                                        print("(self.last_encoder_3) + (self.t_enc)",(self.last_encoder_3) + (self.t_enc),"self.count_3",self.count_3)
                                        self.count_19 += 1 
                                        if self.count_19 <= 15:
                                            self.vel_pub.publish(0)
                                            self.ste_pub.publish(self.home_value)
                                        elif self.count_19 > 15 and self.count_19 <= 25:
                                            self.vel_pub.publish(0)
                                            self.ste_pub.publish(2000)
                                        else:
                                            self.count_2 = 0
...
#                                    self.take_pallet = 11
                            ...
                            if self.dir_sub == 2:
                                ...
                                if ((self.last_encoder_90_d) + (self.t_enc)) >= (self.enc_90 - 50) and ((self.last_encoder_90_d) + (self.t_enc)) <= (self.enc_90 + 150):
                                    #print("ok here")
                                    if (self.position(self.mag_ss_front) >= 2 and self.position(self.mag_ss_front) <= 15 and self.no_line_flag_front == 0) or (self.cross_front_detect == 1) :
                                        self.vel_pub.publish(0)
                                        self.ste_pub.publish(self.home_value)
                                        self.front_flag = 0
                                        self.front_flag_ = 0
