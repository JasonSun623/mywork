...
class zone_checker(object):
 ...
 def __init__(self):
  ...
  self.zoneArea = np.array([[-6.3, 4,  -6.3, -4.5, -19.3, 4], \
  [16, 5, 16, -3, 3.4, 5], [17, -5.1, 17, -7.1, 11, -5.1], \
  [10.7, -5.1, 10.7, -7.1, -7, -5.1], [1, -0.5, 1, -2.5, -3.4, -0.5], \
  [-3.5, -0.5, -3.5, -2.5, -6.2, -0.5], [3.3, -0.5, 3.3, -2.5, 1.1, -0.5], \
  [11, -3.1, 11, -5, 9, -3.1], [-7.1, -5.1, -7.1, -7.1, -11.5, -5.1]])
    ...
    for x in range(0, len(self.zoneArea)):
        ...
        if ((0 <= condition_1)and(condition_1 <= p21_magnitude)):
            if ((0 <= condition_2)and(condition_2 <= p41_magnitude)):
                #print("Inside")
                if (x == 0):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Docking\"}"
                elif (x == 1):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Put_away\"}"
                elif (x == 2):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Ready\"}"
                elif (x == 3):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Highway1\"}"
                elif (x == 4):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Highway2\"}"
                elif (x == 5):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Docking_Highway2\"}"
                elif (x == 6):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Highway2_Put_away\"}"
                elif (x == 7):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Put_away_Ready/Highway1\"}"
                elif (x == 8):
                    self.zoneMsg = "{\"robot\": \"Robot_3rd\", \"zone\":\"Highway1_Docking\"}"
                if (x_ != 0)and(y_ != 0):
