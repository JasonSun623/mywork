...
class line_follow():
    ...
    def pid_cal(self,pos,kp):#P only
        if pos == 8 :
            proportional = pos - self.set_point
            ster_value = int(proportional* kp)# + self.integral*ki)# + derivative*kd)
            return ster_value
        elif pos > 8 and pos <= 11 :
            proportional = 2#pos - self.set_point
            #if proportional
            ster_value = int(proportional* kp)# + self.integral*ki)# + derivative*kd)
            return ster_value
        elif pos >= 11  :
    ...
    #####################__Put_down_pallet__#####################
    def put_down_pallet(self,speed):
        print self.pid_cal(self.position(self.mag_ss),4)
        turning_value = int(self.pid_cal(self.position(self.mag_ss),4))
        #print type(turning_value)
