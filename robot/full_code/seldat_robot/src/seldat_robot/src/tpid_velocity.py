...
class PidVelocity():
	...
	print "Initializing TPID Class..."
        ...
        ### get parameters #### 
        self.Kp = rospy.get_param('~Kp',17000) 	#12000
        self.Ki = rospy.get_param('~Ki',50) 	#50
        self.Kd = rospy.get_param('~Kd',400)	#400

	self.Kp_H = rospy.get_param('~Kp_H',40000)
	self.Ki_H = rospy.get_param('~Ki_H',60)
	self.Kd_H = rospy.get_param('~Kd_H',2000)

        self.out_min = rospy.get_param('~out_min',-200)
	...
	if (abs(self.target) <= 0.5):
		self.motor = (self.Kp * self.error) + (self.Ki * self.integral) + (self.Kd * self.derivative)
		kp_ = (self.Kp * self.error)
        	ki_ = (self.Ki * self.integral)
        	kd_ = (self.Kd * self.derivative)
		#print ("=============Under 0.5=============")
        	#print (kp_)
       		#print (ki_)
        	#print (kd_)
        	...
        	self.motor = (self.Kp_H * self.error) + (self.Ki_H * self.integral) + (self.Kd_H * self.derivative)
		kp_ = (self.Kp_H * self.error)
		ki_ = (self.Ki_H * self.integral)
		kd_ = (self.Kd_H * self.derivative)
		#print ("==============Over 0.5==============")
		#print (kp_)
		#print (ki_)
		#print (kd_)
