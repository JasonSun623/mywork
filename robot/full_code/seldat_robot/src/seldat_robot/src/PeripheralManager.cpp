...
int main(int argc, char *argv[])
{
	...
	ros::Subscriber sub_sensordata= nh.subscribe("SensorCallBack",1,SensorData_callback);
	...
	ros::Subscriber sub_pathfinder= nh.subscribe("path_finder",1,pathfinder_callback);
	ros::Subscriber sub_batteryvol= nh.subscribe("battery_vol",1,batterysub_callback);
	ros::Subscriber sub_batteryhardwarecallback= nh.subscribe("chargeCT",1,batteryhardware_callback);
	ros::Subscriber sub_batteryselfdrivingcallback= nh.subscribe("batteryselfdrivingcallback",1,batteryselfdriving_callback);
	ros::Subscriber sub_serverCtrlcallback= nh.subscribe("serverCtrlCallBack",1,serverCtrlcallback);
	ros::Subscriber sub_chargerCtrlCallBack= nh.subscribe("chargerCtrlCallBack",1,chargerCtrlCallBack);
	// publish command
	...
	pub_navigation_Initpose=nh.advertise<geometry_msgs::PoseWithCovarianceStamped>("initialpose", 1000);
	...
	pub_responseInf=nh.advertise<std_msgs::String>("serverRequest", 1000);
	pub_manualctrl=nh.advertise<std_msgs::String>("ManualCtrl", 1000);
	...
	pub_getsensordata=nh.advertise<std_msgs::String>("GetSensorData", 1000);
	pub_selfdriving=nh.advertise<std_msgs::String>("RequestedProgress_SelfDriving", 1000);
	//pub_RequestCmdServer=nh.advertise<std_msgs::String>("serverRequest", 1000);
	pub_ctrlRobotHardware=nh.advertise<std_msgs::Int32>("charge_cmd", 1000);
	pub_BatteryChargeRequestSelfDriving=nh.advertise<std_msgs::String>("BatteryChargeRequestSelfDriving", 1000);
	pub_peripheralManagerToCharger=nh.advertise<std_msgs::String>("peripheralManagerToCharger", 1000);
