...
class line_follow():
    ...
    def Mag_callback(self,msg):
        ...
        config.mag_ss = map(int,mag_value_not)
    ...
    def angle_controll(self,speed):
        turning_value = pid_.pid_cal(pos.position(config.mag_ss),55,80)#35,5
        ...
        if self.angle > self.home_value: 
            self.angle += 10
        if self.angle < self.home_value:#increese angle when < home_value
            self.angle -= 100
        else:
            self.angle = self.angle
        print "self.angle",self.angle
        if self.angle > self.home_value + 200 and self.angle <= self.home_value + 350:
            self.angle +=20
            if speed < 0:
                speed = speed + 50
            elif speed == 0:
            ...
            if speed < 0:
                ...
                speed = speed - 200
        elif self.angle < self.home_value - 150 and self.angle >= self.home_value - 350:
            self.angle -=50
            if speed < 0:
                speed = speed + 50
            elif speed == 0:
